...
DROP PROCEDURE IF EXISTS `pr_del_xltemplate` $$

CREATE PROCEDURE `pr_del_xltemplate`(
...
me:BEGIN
  ...
  if not exists(select xltemplate_gid from sms_mst_txltemplate
    where xltemplate_gid = in_xl_template_gid
    and delete_flag = 'N') then
    set err_msg := concat(err_msg,'Invalid xl template,');
    set err_flag := true;
  end if;

  if exists(select xltemplate_gid from sms_trn_tfile
    where xltemplate_gid = in_xl_template_gid
    and delete_flag = 'N') then
